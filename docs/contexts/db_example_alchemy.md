...
orchestration and repository access, leaving only ORM models,
repo/service subclasses, DTOs, controllers, and config.

```
AuthorController                     BookController
 /db-example-alchemy/authors          /db-example-alchemy/books
          |                                    |
...
|---|---|---|---|---|
| POST | `/db-example-alchemy/authors` | `AuthorWriteDTO` | `AuthorReadDTO` | 201 |
| POST | `/db-example-alchemy/authors/bulk` | `list[AuthorModel]` (collection) | `Sequence[AuthorModel]` | 201 |
| GET | `/db-example-alchemy/authors` | — | `OffsetPagination[AuthorModel]` | 200 |
| GET | `/db-example-alchemy/authors/{author_id:uuid}` | — | `AuthorReadDTO` | 200 |
| PATCH | `/db-example-alchemy/authors/{author_id:uuid}` | `AuthorPatchDTO` | `AuthorReadDTO` | 200 |
| DELETE | `/db-example-alchemy/authors/{author_id:uuid}` | — | — | 204 |
| POST | `/db-example-alchemy/books` | `BookWriteDTO` | `BookReadDTO` | 201 |
| GET | `/db-example-alchemy/books` | — | `OffsetPagination[BookModel]` | 200 |
...
### Config (`DB_EXAMPLE_ALCHEMY_` prefix)

| Env var | Default | Notes |
|---|---|---|
| `DB_EXAMPLE_ALCHEMY_DB_PATH` | `${VOLUME_PATH}/db_example_alchemy.db` | Relative paths resolved under `VOLUME_PATH` |
...
`ports/driving/` DTOs and `ports/driven/services/`.
...
Production contexts should use yoyo (see `db_example`) or Alembic.
...
`db_example_alchemy` is the **only** SQLAlchemy user in the template.
`db_example`, `admin/log`, and all other contexts use aiosqlite directly.
...
- `src/db_example_alchemy/` — full context source
- [docs/adr/0012-db-example-alchemy-advanced-alchemy-dishka.md](../adr/0012-db-example-alchemy-advanced-alchemy-dishka.md) — advanced-alchemy + hybrid layering + create_all decision
- [docs/adr/0013-litestar-2.23-floor.md](../adr/0013-litestar-2.23-floor.md) — version bump rationale
- [docs/contexts/db_example.md](db_example.md) — raw aiosqlite counterpart
