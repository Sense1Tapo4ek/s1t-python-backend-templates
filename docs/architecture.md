...
| `auth` | `src/litestar_backend/src/auth/` | Auth: three credential families (JWT, API-key, static admin token) resolved by a composite chain behind `AuthMiddleware`, plus `require_role` guards. Owns a Postgres schema (`api_keys`) migrated on startup. See [litestar_backend/docs/contexts/auth.md](../src/litestar_backend/docs/contexts/auth.md), [subsystems/jwt-auth.md](../src/litestar_backend/docs/subsystems/jwt-auth.md). |
