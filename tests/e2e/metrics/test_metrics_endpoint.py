"""E2E tests for the /metrics Prometheus endpoint."""
...
class TestMetricsBodyContent:
    def test_http_metric_names_appear_after_request(
        ...
        Then the Prometheus body contains HTTP counter/histogram metric names
        with the app_name prefix (dashes replaced by underscores).

        Uses the module-scoped client (APP_NAME=test-service) so the
        PrometheusMiddleware metrics are already populated from the /health
        warm-up call made by the e2e_client fixture.
        """
        resp = e2e_client.get("/metrics", headers=e2e_auth_headers)
        assert resp.status_code == 200
        body = resp.text
        # Litestar PrometheusMiddleware uses APP_NAME as metric prefix
        # (prometheus_client replaces dashes with underscores).
        prefix = "test_service"
        assert any(
